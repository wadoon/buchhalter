...
class MainWindow(QtGui.QMainWindow, Ui_MainWindow):
	...
	self.setWindowTitle("Buchhalter - %s" % __version__)
...
## SLOT:    
    ...
    @pyqtSignature('')
    def on_btnExportAccounts_clicked(self):
	filter="Html (*.html);; Text (*.txt)"
	filn = str(QtGui.QFileDialog.getSaveFileName ( self, "Export Accounts", ".", \
	    filter,filter))
	if filn:
		...
		if filn.endswith("html"):
		    fil.write(self.viewAccounts.page().mainFrame().toHtml())
